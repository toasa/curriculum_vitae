...
### Android のグラフィクスに関する調査 (2024年10月 - 現在)
...
- ポイント
  ...
  - 動画 (フルHD@60fps) 再生時の性能評価
    ...
    - [FrameTimeline を用いたジャンク検出機能](https://perfetto.dev/docs/data-sources/frametimeline)の使用。特に、60fps の動画生成で合成時間が 16.6 msを越えるにも関わらずジャンクと判定されないフレームが存在したため、ソースコードを読んで原因を調査。結果はスケジューラが想定したフレーム終了時刻と実際の表示時刻の差が一定時間以内であればジャンクと判定されないことがわかった。
- 2名/メンバー
